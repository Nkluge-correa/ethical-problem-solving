...
fileList = glob.glob('./AIA/anti-discrimination/**.yaml')
...
df = pd.DataFrame(
  columns=['question', 'multiple', 'options', 'scores', 'trustworthiness', 'fairness', 'accountability', 'information']
  )
...
score_trustworthiness = 0
score_fairness = 0
score_accountability = 0

# We initialize the maximum scores for each principle
max_trustworthiness = df[df['trustworthiness'] == True].max_impact_increase.sum()
max_fairness = df[df['fairness'] == True].max_impact_increase.sum()
...
<h2><em>Anti-discrimination & Trustworthiness</em></h2>

<p>This <strong>Algorithmic Impact Assessment (AIA)</strong> is a tool that seeks to assess the impact \
  that the application could have on society concerning acts of discrimination that may be automated by\
    artificial intelligence applications, using the ethical principles of <em>Trustworthiness</em>, \
      <em>Fairness</em>, and <em>Accountability</em> as points of reference.</p>

<ul>
  <li><strong>Trustworthiness:</strong> "<em>This principle upholds the idea that AI technologies should be reliable,\
     in the sense that their use can be verifiably attested as safe and robust, promoting user trust and better\
       acceptance of AI technologies.</em>."</li><br>

  <li><strong>Fairness:</strong> "<em>This principle upholds the idea of non-discrimination and bias mitigation.\
     It defends that, regardless of the different sensitive attributes that may characterize an individual,\
       algorithmic treatment should happen fairly</em>."</li><br>
...
for i, page in enumerate(pages):
      ...
      try:
        ...
        if temp_df.trustworthiness.iloc[0]:
            score_trustworthiness += hash_map[page['']['description']]
        
        if temp_df.fairness.iloc[0]:
            score_fairness += hash_map[page['']['description']]
        ...
        for d in page['']:
            ...
            if temp_df.trustworthiness.iloc[0]:
                score_trustworthiness += hash_map[d['description']]
            
            if temp_df.fairness.iloc[0]:
                score_fairness += hash_map[d['description']]
            
            if temp_df.accountability.iloc[0]:
                score_accountability += hash_map[d['description']]   

# We make sure that the scores are not negative
score_trustworthiness = score_trustworthiness if score_trustworthiness > 0 else 0
score_fairness = score_fairness if score_fairness > 0 else 0
...
fig = make_subplots(
    ...
    subplot_titles=("<b>Impact on Trustworthiness</b>","<b>Impact on Fairness</b>", "<b>Impact on Accountability</b>"),
    horizontal_spacing = 0.10,
)

fig.add_trace(go.Indicator(
    mode = "gauge+number",
    value = (score_trustworthiness/max_trustworthiness) * 100,
    domain = {'x': [0, 1], 'y': [0, 1]},
    gauge = {
...
fig.add_trace(go.Indicator(
    mode = "gauge+number",
    value = (score_fairness/max_fairness) * 100,
    domain = {'x': [0, 1], 'y': [0, 1]},
    gauge = {
        'axis': {'range': [None, 100], 'tickwidth': 1, 'tickcolor': "black"},
        'bar': {'color': "slategray"},
...
fig_total = go.Figure(go.Indicator(
    mode = "gauge+number",
    value = (((score_trustworthiness/max_trustworthiness) + 
    (score_fairness/max_fairness) + 
...
fig_total.update_layout(
  ...
  plot_bgcolor='rgba(0,0,0,0)',
  template='plotly_white',
  title=dict(text='<b>Total Impact Score (Anti-discrimination & Trustworthiness)</b>', x=0.5, y=0.9, font={'size': 24}), 
  font = {'color': "black", 'family': "Arial"})
...
<p>This report contains the risk assessment carried out by our tool. Our assessment consists of a set of risk indicators \
  relating to the three principles assessed in the survey: <strong>Trustworthiness</strong>, <strong>Fairness</strong>, \
